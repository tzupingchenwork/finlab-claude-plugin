...
```bash
finlab pm create <portfolio_name> <strategy1> <weight1> <strategy2> <weight2> --total_balance <amount>
```
...
Synchronize portfolio positions with broker account.

```bash
finlab pm sync <portfolio_name> --broker <broker_name>
```

**Example:**
```bash
finlab pm sync 多元投資 --broker sinopac
```
...
Set up scheduled synchronization times.

```bash
finlab schedule pm <portfolio_name> <start_time> <end_time>
```

**Example:**
```bash
finlab schedule pm 多元投資 08:00 20:00
```
